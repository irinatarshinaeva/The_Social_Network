...
import { Button, withStyles, Icon } from '@material-ui/core';
import { chatPrivatThunk, getWrongThunk } from '../../redux/creators/posts';
import {
  allOurMessagesThunk,
  setAllMessages,
} from '../../redux/creators/messages';
      ...
      <div className="chatPage">
        <section className="chatMessages">
          {messages.map((message, index) => {
            return (
              <div
                className={`${message.login === user ? 'myRow' : 'partnerRow'}`}
                key={index}
              >
                <div
                  className={`${
                    message.login === user ? 'myMessage' : 'partnerMessage'
                  }`}
                >
                  {message.login === user ? (
                    <LightSpeed left>{message.message}</LightSpeed>
                  ) : (
                    <LightSpeed right>{message.message}</LightSpeed>
                  )}
                </div>
              </div>
            );
